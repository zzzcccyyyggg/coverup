    ...
    def __init__(self, max_entries: int = 300):
        ...
        self._prescription_templates: Dict[Tuple[str, str], str] = {
            ...
            ("rust", ErrorCategory.TYPE.value):
                ...
                "Use compiler labels literally: if argument #N of type `T` is missing, add it in that position; "
                "if the compiler expects enum `Foo`, compare against `Foo::*`, not a different enum.",
            ...
            ("rust", ErrorCategory.OWNERSHIP.value):
                ...
                "Avoid holding references across assertion points, and bind temporaries with `let` "
                "before borrowing them.",
            ("rust", ErrorCategory.PANIC.value):
                "Do: treat repeated panic/assertion failures as a semantic mismatch. Re-derive the "
                "expected behavior from the implementation, keep one minimal test, and prefer "
                "smaller invariants over guessed full outputs.",
            ("rust", ErrorCategory.UNKNOWN.value):
                "Do: stop patching locally after repeated failures. Use get_info on the main helper "
                "types/functions, rebuild the test from first principles, and remove speculative assertions.",
