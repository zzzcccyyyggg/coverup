...
class GoBackend(LanguageBackend):
    ...
    async def measure_test_coverage(
        ...
        *,
        ...
        prepared_code = self._enforce_test_size(prepared_code, log_write)
    ...
    def save_successful_test(
        ...
        prepared_code = self._prepare_test_code(test_code, segment)
        prepared_code = self._enforce_test_size(prepared_code, None)
        prepared_code = self._ensure_unique_test_names(package_dir, prepared_code)
    ...
    def _compact_test_for_size_limit(self, code: str) -> str:
        """Reduce formatting noise when a generated test is just over the line limit.

        This keeps behavior intact while reclaiming space from comment-only lines
        and repeated blank lines that often accumulate after multiple repair passes.
        """
    ...
    def _enforce_test_size(
        ...
        *,
        ...
        compacted = self._compact_test_for_size_limit(code)
        if compacted != code:
            compacted_line_count = compacted.count("\n") + 1
            compacted_char_count = len(compacted)
            if log_write:
                log_write(
                    "[coverup] compacted generated Go test for size control: "
                    f"{line_count} lines / {char_count} chars -> "
                    f"{compacted_line_count} lines / {compacted_char_count} chars.\n"
                )
            if compacted_line_count <= max_lines and compacted_char_count <= max_chars:
                return compacted
            code = compacted
            line_count = compacted_line_count
            char_count = compacted_char_count
