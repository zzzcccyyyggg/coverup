...
| Seeds     | 42, 123, 456 | 3 |
| Variants  | full, no_memory, no_repair, no_planner, baseline | 5 |
| **Total** | | **135 runs** |
...
| `baseline` | Vanilla CoverUp (all disabled) | `--no-agent-memory --no-agent-repair --no-agent-planner` |
...
PYTHONPATH=src python3 scripts/run_experiments.py \
    --project similar --variant full baseline --seed 42
```
...
```bash
PYTHONPATH=src python3 scripts/run_experiments.py --resume
```
...
# Ablation deltas
python3 scripts/analyze_results.py --ablation

# Log analysis
python3 scripts/analyze_log.py experiments/logs/similar_full_s42.log
```
